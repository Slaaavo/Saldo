...
interface Props {
  ...
  selectedDate: string
  exclusions: number[]
  onSubmit: (updates: { accountId: number; amountMinor: number }[], eventDate: string, note: string) => Promise<void>
  onClose: () => void
}
  ...
  const renderRow = (row: SnapshotRow) => (
    <React.Fragment key={row.accountId}>
      <div className="flex-col flex">
        ...
        {row.accountType === 'bucket' && <div className="text-xs text-muted-foreground">{t('modals.createBalanceUpdate.extraBalance')}</div>}
        <NumberValue value={row.balanceMinor} currencyCode={row.currencyCode} minorUnits={row.currencyMinorUnits} className="text-xs text-muted-foreground" />
      </div>
      <CurrencyInput
        type="number"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0 gap-4">
          <DialogBody className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              <DatePicker id="bulk-date" value={date} onChange={setDate} withTime defaultTime="23:59" />
            </div>
            ...
            <div className="grid grid-cols-[auto_1fr] items-center gap-x-8 gap-y-3">
              {realAccounts.length > 0 && (
                ...
                >
                  {accountsExpanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              ...
              {bucketAccounts.length > 0 && (
                ...
                >
                  {bucketsExpanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input id="bulk-note" type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder={t('modals.bulkUpdate.notePlaceholder')} />
            </div>
          </DialogBody>
