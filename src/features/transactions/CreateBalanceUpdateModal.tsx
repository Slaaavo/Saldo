...
interface Props {
  ...
  onBucketSubmit?: (accountId: number, amountMinor: number, eventDate: string, note: string | null, linkedAccountIds: number[]) => Promise<void>
  onClose: () => void
}
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0 gap-4">
          <DialogBody className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="cbu-amount">{isBucket ? t('modals.createBalanceUpdate.extraBalance') : t('modals.createBalanceUpdate.amount')}</Label>
              <CurrencyInput
                id="cbu-amount"
                type="number"
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input id="cbu-note" type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder={t('modals.createBalanceUpdate.notePlaceholder')} />
            </div>
