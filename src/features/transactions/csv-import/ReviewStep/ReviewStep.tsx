...
export interface ReviewStepProps {
  ...
  onLegPartnerChange: (rowIndex: number, legIndex: number, rawIban: string | null, ibanMatch: IbanMatchResult, counterpartAccountId: number | null) => void
  ...
  return (
    <>
      <div className="flex flex-col gap-3 min-h-0">
        <p className="text-sm text-muted-foreground">{t('import.reviewStep.summary', { total, duplicates: duplicateCount })}</p>
        ...
        <div className="flex items-center gap-2">
          ...
          <label htmlFor="select-all" className="text-sm cursor-pointer select-none">
            {selectedCount > 0 ? t('import.reviewStep.selectAll') + ` (${selectedCount}/${total})` : t('import.reviewStep.selectAll')}
          </label>
        </div>
          ...
          <table className="w-full text-sm table-fixed border-separate border-spacing-0">
            ...
            <thead>
              <tr>
                ...
                <th className="px-2 py-2 text-left font-medium text-muted-foreground whitespace-nowrap border-b border-border">{t('import.mappingStep.field.date')}</th>
                <th className="px-2 py-2 text-right font-medium text-muted-foreground whitespace-nowrap border-b border-border">{t('import.mappingStep.field.amount')}</th>
                <th className="px-2 py-2 text-left font-medium text-muted-foreground border-b border-border">{t('import.mappingStep.field.partner')}</th>
                <th className="px-2 py-2 text-left font-medium text-muted-foreground border-b border-border">{t('import.mappingStep.field.note')}</th>
                <th className="px-2 py-2 text-left font-medium text-muted-foreground border-b border-border">{t('import.reviewStep.bucket')}</th>
                <th className="px-2 py-2 text-left font-medium text-muted-foreground border-b border-border">{t('import.reviewStep.split.actionsColumn')}</th>
              </tr>
            </thead>
          </table>
                  ...
                  return (
                    <Fragment key={row.index}>
                      <tr
                        className={cn(
                          'group align-top',
                          (row.isDuplicate || row.nearDateDuplicateEventId !== null) && 'bg-amber-50 dark:bg-amber-950/20',
                          !row.isSelected && 'opacity-50',
                          isSplit && 'bg-muted/30',
                        )}
                      >
                        <td className="px-2 py-2 border-b border-border group-last:border-b-0">
                          <Checkbox checked={row.isSelected} onCheckedChange={() => onToggleRow(row.index)} />
                        </td>
                        <td className="px-2 py-2 whitespace-nowrap border-b border-border group-last:border-b-0">
                          <div className="flex flex-col gap-0.5">
                            ...
                            {row.isDuplicate && <span className="text-xs font-medium text-amber-600 dark:text-amber-400">{t('import.reviewStep.duplicate')}</span>}
                            {row.nearDateDuplicateEventId !== null && (
                              <span className="text-xs font-medium text-amber-600 dark:text-amber-400">{t('import.reviewStep.duplicateNearDate')}</span>
                            )}
                          </div>
                        </td>
                        <td className="px-2 py-2 text-right whitespace-nowrap border-b border-border group-last:border-b-0">
                          <NumberValue value={row.amountMinor} minorUnits={selectedAccountMinorUnits} currencyCode={selectedAccountCurrencyCode} />
                        </td>
                        <td className="px-2 py-2 border-b border-border group-last:border-b-0">
                          {isSplit ? (
                        ...
                        <td className="px-2 py-2 border-b border-border group-last:border-b-0">
                          {isSplit ? (
                            ...
                            <Select
                              ...
                              onValueChange={(v) => onBucketChange(row.index, v === BUCKET_NONE ? null : Number(v))}
                            >
                        ...
                        <td className="px-2 py-2 border-b border-border group-last:border-b-0">
                          {isSplit ? (
                            <Button type="button" variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={() => onSplitCancel(row.index)}>
                              {t('import.reviewStep.split.cancelButton')}
                            </Button>
                          ) : (
                            <Button type="button" variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={() => onSplitOpen(row.index)}>
      ...
      <DialogFooter>
        ...
        <div className="flex flex-col items-end gap-1">
          <Button type="button" onClick={onImport} disabled={(selectedCount === 0 && nearDateSkippedCount === 0) || importing || splitValidationErrors.length > 0}>
            {importing ? '...' : t('import.reviewStep.importButton', { count: selectedCount })}
          </Button>
          {nearDateSkippedCount > 0 && <p className="text-xs text-muted-foreground">{t('import.reviewStep.updateTransfersNote', { count: nearDateSkippedCount })}</p>}
        </div>
      </DialogFooter>
    </>
  )
