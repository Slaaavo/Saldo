...
export function SplitLegRow({
  ...
  const [amountStr, setAmountStr] = useState(() =>
    fromMinorUnits(leg.amountMinor, selectedAccountMinorUnits),
  );
  ...
  return (
    <tr className="align-top">
      ...
      <td className="px-2 py-1.5 border-b border-border/50 text-xs text-muted-foreground whitespace-nowrap">
        {t('import.reviewStep.split.splitLabel', { n: legNumber })}
      </td>
      <td className="px-2 py-1.5 border-b border-border/50">
        <CurrencyInput
          type="text"
      ...
      <td className="px-2 py-1.5 border-b border-border/50">
        <Input
          ...
          value={leg.note ?? ''}
          onChange={(e) => onNoteChange(leg.legIndex, e.target.value || null)}
        />
      </td>
      <td className="px-2 py-1.5 border-b border-border/50">
        <Select
          value={leg.bucketId !== null ? String(leg.bucketId) : BUCKET_NONE}
          onValueChange={(v) => onBucketChange(leg.legIndex, v === BUCKET_NONE ? null : Number(v))}
        >
