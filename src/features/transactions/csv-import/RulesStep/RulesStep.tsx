              ...
              <li key={index} className="flex flex-col gap-2 rounded-[var(--radius)] border p-3">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-semibold text-muted-foreground">
                    {index + 1}. {rule.type === 'sign_from_column' ? t('import.rules.signFromColumnLabel') : t('import.rules.overrideDateFromDescriptionLabel')}
                  </span>
                  <div className="flex items-center gap-1">
                    <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleMoveUp(index)} disabled={index === 0}>
                      <ChevronUp className="h-4 w-4" />
                    </Button>
                    <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleMoveDown(index)} disabled={index === rules.length - 1}>
                      <ChevronDown className="h-4 w-4" />
                    </Button>
                    <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => handleDelete(index)}>
                ...
                {rule.type === 'sign_from_column' ? (
                  <SignFromColumnForm rule={rule} csvHeaders={csvHeaders} onChange={(updated: ImportRule) => handleRuleChange(index, updated)} />
                ) : (
        ...
        <div className="flex items-center gap-2">
          <Select value={addType} onValueChange={setAddType}>
            ...
            <SelectContent>
              ...
              <SelectItem value="sign_from_column">{t('import.rules.signFromColumnLabel')}</SelectItem>
              <SelectItem value="override_date_from_description">{t('import.rules.overrideDateFromDescriptionLabel')}</SelectItem>
            </SelectContent>
          </Select>
          <Button type="button" variant="outline" size="sm" onClick={handleAdd} disabled={addType === ADD_NONE}>
