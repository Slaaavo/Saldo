...
export default function SignFromColumnForm({
  rule,
  csvHeaders,
  onChange,
}: SignFromColumnFormProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        ...
        <Select
          value={rule.typeColumn !== '' ? rule.typeColumn : '__none__'}
          onValueChange={(value) =>
            onChange({ ...rule, typeColumn: value === '__none__' ? '' : value })
          }
        >
          <SelectTrigger>
            <SelectValue />
        ...
        {typeColumnMissing && (
          <p className="text-xs text-amber-600 dark:text-amber-400">
            {t('import.rules.unmatchedColumn', { name: rule.typeColumn })}
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <Input
          value={rule.negativeType}
          onChange={(e) => onChange({ ...rule, negativeType: e.target.value })}
        />
      </div>
    </div>
  );
