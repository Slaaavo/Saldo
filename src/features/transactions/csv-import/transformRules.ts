  ...
  return csvRows.map((row) => {
    ...
    for (const rule of sortedRules) {
      if (rule.type === 'sign_from_column') {
        ...
        applyOverrideDateFromDescription(
          transformed,
          rule as typeof rule & OverrideDateFromDescriptionParams,
        );
      }
    }
...
function applySignFromColumn(
  row: TransformedCsvRow,
  rule: { typeColumn: string; negativeType: string },
): void {
...
function applyOverrideDateFromDescription(
  row: TransformedCsvRow,
  rule: { descriptionColumn: string; conditionRegex: string; dateRegex: string },
): void {
