...
export function useFxRates(onRateUpdated?: () => Promise<void>) {
  ...
  const codeToId = useMemo(() => new Map<string, number>(currencies.map((c) => [c.code, c.id])), [currencies])

  const { dates, targetCurrencies, rateMap } = useMemo(() => buildRatePivot(rates, currencies), [rates, currencies])
