        ...
        <div className="flex items-center gap-3">
          {missingDates.length > 0 && (
            <Button variant="outline" onClick={handleBackfill} disabled={isBackfilling}>
              {isBackfilling ? t('fxRates.backfilling') : t('fxRates.backfill', { count: missingDates.length })}
            </Button>
          )}
          <Label className="text-sm font-medium text-muted-foreground">{t('fxRates.fetchHistorical')}</Label>
            ...
            <thead>
              <tr className="border-b border-border">
                <th className="sticky left-0 z-[1] bg-card text-left py-2 pr-4 font-semibold text-muted-foreground">
                  ...
                  {consolidationCode && <div className="text-xs font-normal">{t('fxRates.baseCurrencyPrefix', { currency: consolidationCode })}</div>}
                </th>
