...
export default function CurrencySelect({ currencies, value, onChange, pinnedCurrencyCodes, className }: CurrencySelectProps) {
  ...
  const pinnedCurrencies =
    pinnedCurrencyCodes && !isSearching ? pinnedCurrencyCodes.map((code) => filteredCurrencies.find((c) => c.code === code)).filter((c): c is Currency => c !== undefined) : []

  const unpinnedCurrencies =
    pinnedCurrencyCodes && !isSearching ? filteredCurrencies.filter((c) => !pinnedCurrencyCodes.includes(c.code)).sort((a, b) => a.code.localeCompare(b.code)) : filteredCurrencies
      ...
      {showDropdown && (
        <div className="absolute z-50 mt-1 w-full max-h-48 overflow-y-auto rounded-md border border-border bg-popover shadow-md">
          {filteredCurrencies.length === 0 ? (
            ...
            filteredCurrencies.map((c) => <CurrencyOption key={c.id} currency={c} onSelect={onChange} />)
          ) : (
            <>
