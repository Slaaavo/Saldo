...
import { getAppSetting, setAppSetting } from '../../shared/api'
...
export const useTheme = () => {
  ...
  const theme: 'light' | 'dark' = preference === 'system' ? (systemDark ? 'dark' : 'light') : preference

  const setThemePreference = (pref: ThemePreference) => {
    setPreference(pref)
    localStorage.setItem('theme', pref)
    setAppSetting('theme', pref).catch((err) => console.error('Failed to save theme setting:', err))
  }
