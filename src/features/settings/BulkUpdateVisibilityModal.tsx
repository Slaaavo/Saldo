...
export default function BulkUpdateVisibilityModal({
  accounts,
  currentExclusions,
  onSave,
  onClose,
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <Checkbox
            ...
            onCheckedChange={() => toggleSection(sectionIds, sectionState)}
          />
        ...
        {sectionAccounts.map((row) => (
          <div key={row.accountId} className="flex items-center gap-2 pl-6">
            <Checkbox
              checked={!excluded.has(row.accountId)}
              onCheckedChange={() => toggleItem(row.accountId)}
            />
          ...
          <DialogBody className="flex flex-col gap-4">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={() => toggleSelectAll(selectAllState)}
              />
              <span className="text-sm font-medium">
                {t('modals.bulkUpdateVisibility.selectAll')}
              </span>
            </div>
