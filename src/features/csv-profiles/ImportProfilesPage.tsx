...
export default function ImportProfilesPage() {
  ...
  const { profiles, loading, deletingProfile, setDeletingProfile, handleDelete } =
    useImportProfiles();
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">
                  {t('import.uploadStep.profile')}
                </th>
                <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">
                  {t('import.profiles.columns')}
                </th>
                <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">
                  {t('import.profiles.rules')}
                </th>
            ...
            <tbody>
              {profiles.map((profile) => (
                <tr
                  key={profile.id}
                  className="border-b border-border last:border-0 even:bg-muted/50"
                >
