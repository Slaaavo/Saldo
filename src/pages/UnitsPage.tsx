 ...
 */
function formatPrice(r: FxRateRow): string {
  try {
 ...
 */
function parsePriceAsRate(input: string): { mantissa: number; exponent: number } | null {
  try {
...
export default function UnitsPage() {
  ...
  if (units.length === 0 && !error) {
    // Still loading or no units — check after load
  }
