...
export default function EditBalanceUpdateModal({ event, accounts, onSubmit, onClose }: Props) {
  ...
  useEffect(() => {
    if (isBucket && event.accountId) {
      void loadAllocations(event.accountId, date);
    } else {
      setAllocations([]);
      setAllocatedTotals(new Map());
      setBackendErrors({});
    }
  }, [event.accountId, date, isBucket, loadAllocations]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isBucket) {
        const bucketId = event.accountId;
      ...
      <DialogContent>
        ...
        >
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={submitting || (isBucket && hasErrors)}>
