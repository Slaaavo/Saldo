...
- **Rounded vs floor integer division:** Use `div_round` from `shared/helpers.rs` when splitting monetary amounts proportionally (depreciation schedules, display estimates like monthly tax burden). Use floor division (`/`) for tax rate applications (`amount * bps / 10000`) — tax law convention is to truncate. Never write inline `(a + b/2) / b`; always call `div_round`.
...
- **Core Tauri commands:** `create_balance_update`, `get_accounts_snapshot`, `list_events`, `create_account`, `update_account`, `delete_account`, `update_event` (inserts new `event_data` row), `delete_event` (soft-delete), `create_taxable_event`, `create_taxable_split_group`, `update_taxable_event`, `update_taxable_split_group`, `delete_split_group`, `calculate_tax_model` (returns `TaxCalculationResult` for a given model_id). See `src-tauri/src/features/` for full signatures.
- **Taxable event validation helpers** (`commands.rs`): `validate_event_type_taxable` (accepts `"revenue"`, `"expense"`, or `"prepaid_expense"`), `validate_expense_only_fields` (rejects non-null deductibility/VAT fields for `revenue` events; accepts both `"expense"` and `"prepaid_expense"`), `validate_taxable_field_ranges` (enforces bps in `[0, 10000]`). All three must be called in both create **and** update command handlers. `prepaid_expense` events also require `validate_prepaid_until` (enforces `prepaid_until` is present and its year is strictly greater than the expense date year).
- **Delete cascade atomicity:** When a command must hard-delete children before soft-deleting a parent, wrap **both** operations in a single outer `with_savepoint_app` in the command handler. Never rely on each individual repository call having its own savepoint — if the parent soft-delete fails after children are already hard-deleted, the data is left in a broken state.
...
- **File-over-folder resolution:** When migrating a flat `Foo.tsx` to a `Foo/` folder, TypeScript resolves `Foo.tsx` before `Foo/index.ts`. If the old flat file is kept (even as a re-export shim), it shadows the barrel. Delete the flat file to make the folder's `index.ts` the canonical public entry point — callers importing `'..../Foo'` resolve to `Foo/index.ts` automatically.
...
- **Date inputs:** Always use the `<DatePicker>` component (`src/shared/ui/date-picker`) for any date field in a form. Never use `<Input type="date">`. `DatePicker` accepts `value: string | undefined` (ISO date or datetime) and `onChange: (date: string) => void`. Pass `value || undefined` when the state is an empty string so the picker shows the placeholder correctly.
- **Adding a new editable event type checklist:** (a) Add to `validate_event_type_taxable` in `commands.rs`. (b) If expense-like, add to `validate_expense_only_fields` and `resolve_default_taxable_account` in `persons/repository.rs`. (c) Add edit-button condition in `LedgerEventList`. (d) Add a branch in `LedgerPage handleEditEvent` — omitting this causes a silent no-op when the button is clicked. (e) Update `isExpense || isNewType` guards in `EditTaxableEventModal` for expense-like fields. (f) If cascade-delete is needed, add a branch in the `delete_event` command handler wrapped in `with_savepoint_app`.
...
- **`const` arrow functions only:** All React component and hook declarations must use `const` arrow functions — never `function` declarations. The ESLint config enforces this. Example: `export const MyComponent = ({ ... }: Props) => { ... }`, not `export function MyComponent(...)`.
- **No explicit JSX return type annotations:** Do not annotate component return types with `: JSX.Element`, `: JSX.Element | null`, or `: ReactElement`. The project's automatic JSX transform does not expose the bare `JSX` namespace globally, so these annotations produce `Cannot find namespace 'JSX'` compile errors. Omit return types entirely — TypeScript infers them correctly.
...
## 14. Tax calculation patterns

- **Pure-function calculation modules:** Business-critical calculations live in Rust (`calculation.rs` inside the feature folder). These modules contain only pure functions -- no DB access. The repository layer fetches raw data structs; the calculation layer receives them and returns results. This separation makes the core math trivially unit-testable with no DB setup needed.
- **`hasNoEvents` / empty result detection:** To detect whether a tax model result has no underlying events, check `result.eventBreakdowns.length === 0`. Do NOT use zero-value checks on income or expense fields -- a person can have expense events with zero income (non-deductible costs only), causing a false positive.

## 15. eKasa receipt import — QR decoding

- **QR decode pipeline** (in `qr_decoder.rs`): Three-step fallback: (1) raw grayscale on full image → rqrr, (2) crop bottom 40% → grayscale → rqrr, (3) crop bottom 40% → Otsu binarization → rqrr. Low-contrast thermal receipt scans need step 3 — rqrr finds the grid but fails ECC on noisy grayscale.
- **PDF multi-resolution rendering:** Render with PDFium at candidate widths `[1500, 2000, 2500]` and return the first successful QR decode. Certain render widths cause pixel aliasing that corrupts QR modules; failures are non-monotonic across widths. Never use a single fixed width.
- **PDFium singleton:** `Pdfium::bind_to_library()` must be called exactly once per process. Use `OnceLock<Result<Pdfium, String>>` to lazily initialize and return `&'static Pdfium`. The `thread_safe` feature is enabled in Cargo.toml.
- **Otsu binarization** (`otsu_binarize` in `qr_decoder.rs`): Computes the optimal threshold via histogram analysis (minimizes intra-class variance), then maps every pixel to pure black or white. No external dependency — implemented inline.

## 16. CSS Grid alignment across independent containers

- When header and data rows are separate CSS Grid containers sharing the same `grid-cols-[...]` template, `auto` columns resolve independently per container. If the header row has no content in those columns but data rows have icon buttons, the columns will have different widths, misaligning labels. Fix: use fixed column widths matching the actual content size (e.g., `2.5rem` for `size="icon"` buttons that render as `w-10`).
